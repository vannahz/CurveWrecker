...
public class CalendarGridViewAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		LinearLayout layout = (LinearLayout) LayoutInflater.from(mContext).inflate(R.layout.calendar_item_layout, null);
		TextView textView = (TextView) layout.findViewById(R.id.calendar_item_tv_day);
		TextView tv_tip = (TextView) layout.findViewById(R.id.calendar_tip);
		ImageView is_first = (ImageView) layout.findViewById(R.id.is_fisrt);
		ImageView is_star = (ImageView) layout.findViewById(R.id.is_star);
		ImageView is_moon = (ImageView)layout.findViewById(R.id.is_moon);
		...
		if (mDataList != null) {
			...
			if ((TextUtils.equals(CalendarTool.SATURDAY, mDataList.get(position).weekDay))
				...
				if (((mDataList.get(position).year == 2015 && mDataList.get(position).month <= 8)
						|| (mDataList.get(position).year < 2015 && mDataList.get(position).month <= 12))
				...
				{// 2015.8以前，且日期在当月
					if (mDataList.get(position).day > 0
						...
						tv_tip.setBackgroundColor(mRes.getColor(R.color.tip_normal));
					} else if (mDataList.get(position).day == 21
							|| mDataList.get(position).day == 22) {
						tv_tip.setBackgroundColor(mRes.getColor(R.color.tip_leave));
					} else if (mDataList.get(position).day == 23
							|| mDataList.get(position).day == 24) {
						tv_tip.setBackgroundColor(mRes.getColor(R.color.tip_late));
					} else {
						tv_tip.setBackgroundColor(mRes.getColor(R.color.tip_normal));
						if (mDataList.get(position).month == 8&& mDataList.get(position).day >= 25) {
							tv_tip.setBackgroundColor(mRes.getColor(R.color.white));
						}
					}
				}
			}
			if (mDataList.get(position).isNowDate&& mDataList.get(position).isSelfMonthDate) {
